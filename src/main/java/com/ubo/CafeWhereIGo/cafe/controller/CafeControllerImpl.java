...
import com.ubo.CafeWhereIGo.cafe.cafePhotoVO.service.CafePhotoService;
import com.ubo.CafeWhereIGo.cafe.cafePhotoVO.vo.CafePhotoVO;
...
import com.ubo.CafeWhereIGo.cafe.service.CafeService;
...
public class CafeControllerImpl implements CafeController {
	...
	public ModelAndView addCafe(MultipartHttpServletRequest multipartRequest, HttpServletResponse response) throws Exception {
		...
		cafeService.addCafe(cafeInfo);
		
		//get Cafe ID that registered recently
		int cafe_id = cafeService.getRecentCafeId();
		...
		cafeService.addFacilityInfo(facilityInfo);
		...
		if(homeArticleContent!=null && !homeArticleContent.equals("")) {
			...
			cafeService.setHomeArticle(homeArticle);
		}
		...
		if(parkingLotList.size()>0) cafeService.setParkingLotInfo(parkingLotList);
		
		/*
		 * add Goods
		 * */
		//save Goods Info
		List<GoodsPhotoInfoVO> goodsImageFileInfoList = goodsService.addGoods(multipartRequest, cafe_id, goodsSize);
		//save Goods Image
		 ...
		 * */
		//save GroupSeat Info
		List<GroupSeatPhotoInfoVO> groupSeatImageFileInfoList = groupSeatService.addGroupSeat(multipartRequest, cafe_id, goodsSize);
		//save Goods Image
	...
	private void cafeMainPhotoUpload(MultipartHttpServletRequest multipartRequest, int cafe_id) throws IllegalStateException, IOException {		
		...
		if(!orgFilenameWithUUID.equals("") || orgFilenameWithUUID!=null) {
			...
			if(mFile.getSize()>0) {
				...
				CafePhotoVO cafePhoto = new CafePhotoVO(cafe_id, orgFilenameWithUUID, photoType);
