...
<c:set var="now" value="${article.created_date}"/> 
<c:set var="img_maximum_per_td" value="12"/>
<c:set var="img_rownum_temp" value="${articlePhotoList.size()/img_maximum_per_td}"/>
<c:set var="img_rownum" value="${img_rownum_temp+(1-img_rownum_temp%1)%1}"/>
<!DOCTYPE html>
<html>
	<head>
		<meta charset="UTF-8">
		<title>카페어디가-Q&A</title>
		...
		<script>
			...
			function add_img_input(){
				...
				var img_maximum_per_td=${img_maximum_per_td};
	...
	<body>
		<section id="mainContainer">
			<div id="mainContentWrapper">
				...
				<div id="formWrapper">
					<form method="post" action="${contextPath}/cafeTourReview/modifyArticle.do" enctype="multipart/form-data">
						<div id="formContentWrapper">
							<div id="top_formWrapper">
								...
								<input id="text_input" name="article_title" class="text_input article_subject" type="text" value="${article.article_title}" placeholder="글제목" required>
							...
							<hr id="add_aricle_hr">
							<div>
								<textarea id="textarea" name="article_body" rows="50" cols="100"><c:out value="${article.article_content}"/></textarea>
							</div>
						</div>
						...
						<div id="preview_img_wrapper">
							<table id="preview_img_table">
								<c:set var="photoIterator" value="${articlePhotoList.iterator()}"/>
								<c:forEach begin="1" end="${img_rownum}" step="1" varStatus="row">
									<c:set var="rownum" value="${row.index}"/>
									<tr id="imgPreview_row${rownum}">
										
										<c:forEach begin="1" end="${img_maximum_per_td}" step="1" varStatus="col">
											<c:set var="colnum" value="${col.index}"/>
											<c:if test="${(rownum*colnum)<=articlePhotoList.size()}">
												<td id="imgPreview${rownum*colnum}_td">
													<c:set var="photo" value="${photoIterator.next()}"/>
													<img id="img${rownum*colnum}_preview" class="previewImg_prev" src="${contextPath}/article/download.do?article_id=${photo.article_article_id}&fileName=${photo.filename}"/>
												</td>
											</c:if>
										</c:forEach>
									</tr>
								</c:forEach>
								<c:remove var="photoIterator"/>								
							</table>
						</div>
