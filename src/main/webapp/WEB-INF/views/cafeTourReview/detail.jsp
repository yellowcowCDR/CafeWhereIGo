	...
	<head>
		...
		<script>
					...
					<c:if test="${loginSession != null}">
						...
						if(!isLike){					
							$.ajax({
								...
								url: "${contextPath}/cafeTourReview/like.do",
			...
			function addReply(){
				<c:if test="${loginSession != null}">
					...
					$.ajax({
						...
						url: "${contextPath}/cafeTourReview/addReply.do",
						success: function(data){
							location.reload();
						},
						error:function(jqXHR, textStatus, errorThrown){
			...
			function goToModifyForm(){
				<c:if test="${loginSession != null}">
					if("${loginSession.user_id}" == article_writer_id){
						var url = "${contextPath}/cafeTourReview/modifyArticleForm.do?article_id=${article.article_id}"	
						location.href=url;
					}else{
			...
			function articleDelete(){
				<c:if test="${loginSession != null}">
					if("${loginSession.user_id}" == article_writer_id){
						var url = "${contextPath}/cafeTourReview/deleteArticle.do?article_id=${article.article_id}"	
						location.href=url;
					}else{
