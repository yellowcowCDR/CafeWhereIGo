					...
					<form>
						<div id="formContentWrapper">
							...
							<c:if test="${orderList!=null}">
								<div id="orderCancelContainer"> 
									<div align="center" id="orderCancel" class="align-right">
										<button type="button" class="blueButton" onclick="toggleGoodsOrderSelect()">주문전체선택/취소</button>
									</div>	
								</div>
									...
									<c:forEach var="order" items="${orderList}" varStatus="status">
										<c:if test="${status.index>0}">
											...
											<tr name='goodsOrderRow'>
												...
												<td class="takeout_check_td" style="text-align:center">
										       ...
										       <td width="100px" style="text-align:center">
										       		<p class="price-text no-top_bottom_margin">총 ${order.order_quantity * order.goods_price}원</p>
										       		<input type="hidden" name="goods_unit_totalPrice" value="${order.order_quantity * order.goods_price}">
									       		</td>
									       		<td width="100px" style="text-align:center">
													<p class="no-top_bottom_margin">
														<c:if test="${order.order_state == 'complete'}">
															주문완료
														</c:if>
														<c:if test="${order.order_state == 'canceled'}">
															주문취소
														</c:if>
													</p>
													<input type="hidden" name="order_state" value="${order.order_state}">													
												</td>
										       <td width="28px"><input type="button" class="orderModifyButton" onclick="modifyGoodsOrder('${contextPath}',this.parentNode.parentNode)" value="변경"></td>
										       <td width="28px"><input type="button" class="orderDeleteButton" onclick="cancelGoodsOrder('${contextPath}',${order.order_id})" value="취소"></td>
							...
							<c:if test="${reservationList!=null && reservationList.size()>0}">
								<div id="orderCancelContainer"> 
									<div align="center" id="orderCancel" class="align-right">
										<button type="button" class="blueButton" onclick="toggleResrvationSelect()">예약전체선택/취소</button>
									</div>	
								</div>
								<table align="center" id="reservationTable">
									...
									<c:forEach var="reservation" items="${reservationList}" varStatus="status">
										...
										<tr name='reservationRow'>
											<td width="13px"><input class="checkboxInput" name="reservationChecked" type="checkbox"></td>
											<td width="80px">
											...
											<td width="100px" style="text-align:center">
												<p class="price-text no-top_bottom_margin">총 ${reservation.totalPrice}원</p>
												<input type="hidden" name="groupSeat_unit_totalPrice" value="${reservation.totalPrice}">
											</td>
											<td width="100px" style="text-align:center">
												<p class="no-top_bottom_margin">
													<c:if test="${reservation.reservation_state == 'complete'}">
														예약완료
													</c:if>
													<c:if test="${reservation.reservation_state == 'canceled'}">
														예약취소
													</c:if>
												</p>
												<input type="hidden" name="reservation_state" value="${reservation.reservation_state}">	
											</td>
											<td width="28px"><input type="button" class="orderModifyButton" onclick="modifyGroupSeatReservation('${contextPath}',this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode)" value="변경"></td>
											<td width="28px"><input type="button" class="orderDeleteButton" onclick="cancelGroupSeatReservation('${contextPath}', ${reservation.reservation_id})" value="취소"></td>
